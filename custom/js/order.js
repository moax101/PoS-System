...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
			...
			if(custName == "") {
				$("#custName").after('<p class="text-danger"> The Customer Name field is required </p>');
				$('#custName').closest('.form-group').addClass('has-error');
			} else {
				$('#custName').closest('.form-group').addClass('has-success');
			} // /else			

			if(price == "") {
				$("#price").after('<p class="text-danger"> The Price field is required </p>');
			...
			var validateProduct;
			for (var x = 0; x < menuName.length; x++) {       			
				var menuNameId = menuName[x].id;	    	
		    if(menuName[x].value == ''){	    		    	
		    	$("#"+menuNameId+"").after('<p class="text-danger"> Menu Name Field is required!! </p>');
		    	$("#"+menuNameId+"").closest('.form-group').addClass('has-error');	    		    	    	
	      } else {      	
		    	$("#"+menuNameId+"").closest('.form-group').addClass('has-success');	    		    		    	
	      }          
	   	} // for
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && custName && paid && discount ) {
				if(validateProduct == true && validateQuantity == true) {
	            	...
	            	'<a href="orders.php?o=add" class="btn btn-default" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> Add New Order </a>'+'</div>');
		...
		$("#editOrderForm").unbind('submit').bind('submit', function() {
			...
			for (var x = 0; x < menuName.length; x++) {       			
				var menuNameId = menuName[x].id;	    	
		    if(menuName[x].value == ''){	    		    	
		    	$("#"+menuNameId+"").after('<p class="text-danger"> Product Name Field is required!! </p>');
		    	$("#"+menuNameId+"").closest('.form-group').addClass('has-error');	    		    	    	
	      } else {      	
		    	$("#"+menuNameId+"").closest('.form-group').addClass('has-success');	    		    		    	
	      }          
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				'<td>'+
					...
					'<select class="form-control" name="menuName[]" id="menuName'+count+'" onchange="getProductData('+count+')" >'+
...
function getProductData(row = null) {
	if(row) {
		...
		if(menuId == "") {
			...
			// 	var productValue = $("#menuName"+row).val()

			// 	if($("#menuName"+count).val() == "") {					
			// 		$("#menuName"+count).find("#changeProduct"+productId).removeClass('div-hide');	
			...
			$.ajax({
				...
				success:function(response) {
					...
					// 	if($("#menuName"+count).val() != productValue) {
					// 		// $("#menuName"+count+" #changeProduct"+count).addClass('div-hide');	
					// 		$("#menuName"+count).find("#changeProduct"+productId).addClass('div-hide');								
					// 		console.log("#changeProduct"+count);
					// 	}											
					// } // /for
...
function paidAmount() {
	...
	if(total) {
		var dueAmount = Number($("#paid").val()) + Number($("#discount").val()) - Number(total) ;
	...
	if(orderId) {
		...
		$.ajax({
			...
			success:function(response) {				
				...
				// due 
				$("#paid").val(response.order[10]);				

				// pay amount 
				$("#totamAmount").val(response.order[10]);
				...
				$("#updatePaymentOrderBtn").unbind('click').bind('click', function() {
					...
					if(totalAmount == "") {
						$("#totalAmount").after('<p class="text-danger">The Pay Amount field is required</p>');
					...
					if(discount == "") {
						$("#discount").after('<p class="text-danger">The Pay Amount field is required</p>');
						$("#discount").closest('.form-group').addClass('has-error');
					} else {
						$("#discount").closest('.form-group').addClass('has-success');
					}
					...
					if(paid == "") {
						$("#paid").after('<p class="text-danger">The Pay Amount field is required</p>');
						$("#paid").closest('.form-group').addClass('has-error');
					} else {
						$("#paid").closest('.form-group').addClass('has-success');
					}
					...
					if(change == "") {
						$("#due").after('<p class="text-danger">The Pay Amount field is required</p>');
						$("#due").closest('.form-group').addClass('has-error');
					} else {
						$("#due").closest('.form-group').addClass('has-success');
					}

					if(totalAmount && discount && paid && change) {
